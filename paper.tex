...
\subsubsection{Hypothesis 4: Non-Monotonic Relationship with $\alpha$ and $\mu$}

\textbf{Hypothesis:} There exists an optimal value $\alpha^*$ such that epidemic size is minimized. Similarly, increasing $\mu$ (clustering) is not universally beneficial—there may be a threshold beyond which clustering increases epidemic size.

\textbf{Reasoning:} If $\alpha$ is too low, isolation is ineffective. If $\alpha$ is too high, the network fragments into many small components before the virus spreads, but once the virus enters a component, it may saturate it completely. There is a balance. Similarly, $\mu = 0$ (no clustering) is our static baseline. Small $\mu$ may be beneficial (maintaining social cohesion), but large $\mu$ creates epidemic traps. The optimal parameter values are an empirical question.

\subsubsection{Hypothesis 5: Observable Network Metrics}

\textbf{Hypothesis:} We will observe the following time-dependent trends in network metrics during an ASIR simulation:

\begin{itemize}[leftmargin=*]
    \item $\langle k(t) \rangle$ will decrease monotonically (edge cutting dominates early when infections are high).
    \item $C(t)$ will increase, particularly in the middle phase of the epidemic (pods form as people isolate from the broader network).
    \item $L(t)$ will initially increase (loss of shortcuts) but may become undefined if the network fragments into disconnected components.
\end{itemize}

These trends constitute direct empirical evidence of co-evolution and will be a major contribution of our work.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Novel Predictions:} We predict a specific, testable pattern—peak reduction with duration extension—that is not obvious from intuition alone. This ``flattening of the curve'' is precisely the public health strategy employed during COVID-19, yet its network-theoretic basis has not been rigorously analyzed in this manner.
    
    \item \textbf{Policy Implications:} Understanding the interplay between individual behavior ($\alpha$, $\mu$) and population outcomes (epidemic size, duration) can inform public health messaging. For example, if our simulations show that $\mu > 0.5$ (aggressive pod formation) is counterproductive, this suggests public health campaigns should emphasize limiting bubble size.
    
    \item \textbf{Network Evolution as Data:} By tracking $\langle k(t) \rangle$, $C(t)$, and $L(t)$ over time, we provide a new lens for understanding epidemics. Future work could use real-world mobility or contact-tracing data to estimate these metrics during actual outbreaks, validating or refining our model.
\end{enumerate}

In the next section, we will implement the ASIR model, run extensive simulations across our three baseline topologies, and test these hypotheses empirically. The results will reveal whether adaptive behavior is a panacea, a pitfall, or a nuanced strategy whose effectiveness depends critically on network structure and parameter choices.
